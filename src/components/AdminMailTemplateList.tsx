...
function EditTemplateForm({ template, locale }: { template: MailTemplate; locale: Locale }) {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(t(locale, 'admin.updateTemplateError'));
        return;
      }
...
export function AdminMailTemplateList({
  ...
  async function remove(id: number) {
    try {
      ...
      if (!res.ok) {
        setError(t(locale, 'admin.deleteTemplateError'));
        return;
      }
      window.location.reload();
    } catch {
  ...
  if (templates.length === 0) {
    return <p className="text-sm text-plexcrew-ash">{t(locale, 'admin.noTemplatesYet')}</p>;
  }
            ...
            >
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
                ...
                <span className="flex flex-none gap-4">
                  <button
                    onClick={() => toggleAction(tpl.id, 'send')}
                    className="rounded font-medium text-plexcrew-ash transition-colors hover:text-plexcrew-screen"
                  >
                    {isActive && activeAction === 'send' ? t(locale, 'admin.close') : t(locale, 'admin.send')}
                  </button>
                  <button
                    onClick={() => toggleAction(tpl.id, 'edit')}
                    className="rounded font-medium text-plexcrew-ash transition-colors hover:text-plexcrew-screen"
                  >
                    {isActive && activeAction === 'edit' ? t(locale, 'admin.close') : t(locale, 'admin.edit')}
                  </button>
                  <button
                    onClick={() => {
                      if (window.confirm(t(locale, 'admin.confirmDeleteTemplate'))) {
                        remove(tpl.id);
                      }
                    }}
              ...
              {isActive && activeAction === 'send' && (
                <div className="border-t border-plexcrew-teal/20 pt-3">
                  <AdminMailComposer template={tpl} members={members} locale={locale} />
                </div>
              )}
              {isActive && activeAction === 'edit' && (
                <div className="border-t border-plexcrew-teal/20 pt-3">
                  <EditTemplateForm template={tpl} locale={locale} />
                </div>
              )}
            </li>
          );
