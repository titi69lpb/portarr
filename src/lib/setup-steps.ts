...
export interface StepResult {
  ok: boolean;
  error: string | null;
  // Stable i18n key + vars for messages generated by Portarr (see ConnectionTestResult).
  errorKey?: string;
...
export async function applyServiceSettings(
  ...
  for (const field of fields) {
    ...
    if (!existing) {
      ...
      return {
        ok: false,
        error: t(DEFAULT_LOCALE, 'settings.errors.fieldRequired', { field: fieldLabel(DEFAULT_LOCALE, field) }),
        errorKey: 'settings.errors.fieldRequired',
        // labelKey (not the rendered label) so the UI can translate it too.
        errorVars: { labelKey: field.label },
      };
    }
