  ...
  return (
    <main className="mx-auto max-w-3xl space-y-8 p-6 sm:p-8">
      <header className="pc-glass-surface-strong sticky top-0 z-10 -mx-6 -mt-6 flex items-center justify-between gap-4 border-b border-plexcrew-teal/20 px-6 py-4 sm:-mx-8 sm:-mt-8 sm:px-8">
        ...
        >
          {t(locale, 'admin.backToPortal')}
        </Link>
      </header>
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <AdminNavCard
          href="/admin/members"
          title={t(locale, 'admin.membersNavTitle')}
          description={t(locale, 'admin.membersNavDesc')}
        />
        <AdminNavCard
          href="/admin/mailings"
          title={t(locale, 'admin.mailingsNavTitle')}
          description={t(locale, 'admin.mailingsNavDesc')}
        />
        <AdminNavCard
          href="/admin/settings"
          title={t(locale, 'admin.settingsNavTitle')}
          description={t(locale, 'admin.settingsNavDesc')}
        />
      </section>
      {config.storageVolumes.length > 0 && (
        <section className="space-y-5">
          ...
          <AdminStorageBars combined={combinedStorage} volumes={volumeStats} locale={locale} />
        </section>
      )}
      <section className="space-y-5">
        <h2 className="text-xs font-semibold uppercase tracking-wider text-plexcrew-ash">
          {t(locale, 'admin.announcements')}
        </h2>
        <AdminAnnouncementForm locale={locale} />
        <AdminAnnouncementList announcements={announcements} locale={locale} />
      </section>
    </main>
  );
}
