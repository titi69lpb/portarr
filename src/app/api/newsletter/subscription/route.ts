...
export async function GET(request: NextRequest) {
  try {
    ...
    const db = getDb();
    return NextResponse.json({ subscribed: isSubscribed(db, { provider: sessionUser.provider, userId: sessionUser.userId }) });
  } catch (err) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const db = getDb();
    setSubscribed(db, { provider: sessionUser.provider, userId: sessionUser.userId }, body.subscribed);
