...
describe('GET /api/dashboard/now-playing', () => {
  it('returns sessions on success', async () => {
    vi.spyOn(global, 'fetch').mockResolvedValue(
      new Response(JSON.stringify({ response: { data: { sessions: [] } } }), { status: 200 })
    );
    const { GET } = await import('../../src/app/api/dashboard/now-playing/route');
